...
*Please refer to the process diagram:* `Process_Diagram_Water_Treatment.png`
...
5. **State Tracking (Bonus):** A dedicated integer file (`N7:0`) actively reports the current valve position to the host system (0 = HOME, 1 = FILL, 2 = DRAIN, 3 = FLUSH, 4 = TRAVELLING).
6. **System Interrupt (Bonus):** A manual reset/interrupt bit (`B3:0/0`) allows operators to abort an active process, bypassing remaining timers and forcing the servo to immediately seek the Home position.
...
| **9. Interrupt Test** | Pulse Cycle, Toggle `B3:0/0`, Force Home OFF, Fill ON. | Servo **REMAINS ENERGIZED** (seeking home). <br> State `N7:0` = **1** | ✅ PASS |
